...
(function (Scratch) {
  ...
  const trampolineFor = (path) => {
    ...
    if (match) {
      return `${TRAMPOLINE}/api/users/${match[1]}`;
    }

    match = parsed.pathname.match(/^\/projects\/(\d+)\/?$/);
    if (match) {
      return `${TRAMPOLINE}/api/projects/${match[1]}`;
    }
  ...
  class ScratchAPI {
        ...
        blocks: [
          ...
          },
          {
            ...
            text: Scratch.translate(
              "[TYPE] of user [USERNAME] limit [LIMIT] offset [OFFSET]"
            ),
            arguments: {
          ...
          },
          {
            ...
            blockType: BlockType.REPORTER,
            text: Scratch.translate(
              "[TYPE] of project [ID] limit [LIMIT] offset [OFFSET]"
            ),
            arguments: {
          ...
          },
          {
            ...
            blockType: BlockType.REPORTER,
            text: Scratch.translate(
              "[TYPE] of studio [ID] limit [LIMIT] offset [OFFSET]"
            ),
            arguments: {
          ...
          { blockType: BlockType.LABEL, text: Scratch.translate("Discover") },
          {
            ...
            blockType: BlockType.REPORTER,
            text: Scratch.translate(
              "search [TYPE] for [QUERY] mode [MODE] limit [LIMIT] offset [OFFSET]"
            ),
            arguments: {
          ...
          },
          {
            ...
            blockType: BlockType.REPORTER,
            text: Scratch.translate(
              "explore [TYPE] mode [MODE] limit [LIMIT] offset [OFFSET]"
            ),
            arguments: {
              TYPE: { type: ArgumentType.STRING, menu: "searchTypes" },
              MODE: { type: ArgumentType.STRING, menu: "modes" },
          ...
          },
          {
            ...
            arguments: {
              SECONDS: { type: ArgumentType.NUMBER, defaultValue: 15 },
            },
          },
          {
            opcode: "clearCache",
    ...
    userField(args) {
      return this.field(
        `/users/${encode(args.USERNAME)}`,
        Cast.toString(args.FIELD)
      );
    }

    userList(args) {
      const type = Cast.toString(args.TYPE);
      if (!["projects", "favorites", "followers", "following"].includes(type)) {
        return "";
      }
      return this.json(
        `/users/${encode(args.USERNAME)}/${type}?limit=${limit(
          args.LIMIT
      ...
      if (type === "comments") {
        ...
        if (!username) {
          this.lastError = Scratch.translate(
            "Could not determine the project's creator."
          );
          this.lastSuccess = false;
          return "";
        }
    ...
    studioList(args) {
      const type = Cast.toString(args.TYPE);
      if (!["projects", "comments", "curators", "managers"].includes(type)) {
        return "";
      }
      return this.json(
        `/studios/${encode(args.ID)}/${type}?limit=${limit(
          args.LIMIT
        )}&offset=${offset(args.OFFSET)}`
      );
    }
    ...
    search(args) {
      const type =
        Cast.toString(args.TYPE) === "studios" ? "studios" : "projects";
      const mode = ["trending", "popular", "recent"].includes(
      ...
      return this.json(
        `/search/${type}?q=${encode(args.QUERY)}&mode=${mode}&limit=${limit(
          args.LIMIT
        )}&offset=${offset(args.OFFSET)}`
      );
    }
    ...
    explore(args) {
      ...
      return this.json(
        `/explore/${type}?mode=${mode}&q=*&limit=${limit(
          args.LIMIT
        )}&offset=${offset(args.OFFSET)}`
      );
    }
