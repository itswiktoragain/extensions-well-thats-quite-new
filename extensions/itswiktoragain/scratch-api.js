...
userLists: {
acceptReporters: true,
items: [item("projects", "projects"), item("favorites", "favorites"), item("followers", "followers"), item("following", "following")],
},
projectFields: {
acceptReporters: true,
items: [
...
if (trampolineURL) {
try {
return await Scratch.fetch(trampolineURL);
} catch {
}
}
const proxyURL = `${CORS_PROXY}${encodeURIComponent(url)}`;
try {
return await Scratch.fetch(proxyURL);
} catch (proxyError) {
const directMessage =
directError instanceof Error ? `${directError.name}: ${directError.message}` : Cast.toString(directError);
const proxyMessage =
proxyError instanceof Error ? `${proxyError.name}: ${proxyError.message}` : Cast.toString(proxyError);
throw new Error(`Direct Scratch API request failed (${directMessage}); CORS relay failed (${proxyMessage})`);
}
}
}
async request(path) {
...
return this.json(`/projects/${encode(args.ID)}/${type}?limit=${limit(args.LIMIT)}&offset=${offset(args.OFFSET)}`);
}
studioField(args) {
